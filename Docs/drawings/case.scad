...
union(){
    difference(){
       ...
       //sottraggo l'anello per inserimento forchetta sx di fissaggio
        ...
        translate([L/2-wir/2,HC-S-S/2,L/4-hir/2])
            linear_extrude(2*S)
                square([wir,hir]);
    ...
    translate([L-3*rr+1.5*rr,S+HC-2*S,S])
    rotate([90,0,-90])
    raccordo(rr,L-3*rr);
    ...
    translate([rr,rr,L-S-S])
        rotate([180,0,-45])
            insertovite(rr,3*rr);
    //Superiore sx
    translate([rr,rr+HC-2*rr,L-S-S])
        rotate([180,0,-135])
            insertovite(rr,3*rr);
    //Superiore dx
    translate([rr+L-2*rr,rr+HC-2*rr,L-S-S])
        rotate([180,0,-45+180])
            insertovite(rr,3*rr);
    //Inferiore dx
